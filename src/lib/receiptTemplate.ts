...
export function generateMonthlyFeeReceiptHTML(data: MonthlyReceiptData): string {
  const dateStr = data.date || new Date().toLocaleDateString("en-IN", {
    day: "numeric",
    month: "short",
    year: "numeric",
  ...
  const amtNum = typeof data.amount === "number" ? data.amount : parseAmt(data.amount);
  const amtFormatted = fmtAmt(amtNum);

  const renderSingleMonthlyReceipt = (copyType: "STUDENT COPY" | "OFFICE COPY") => `
    <div class="receipt-box">
      <!-- Top Brand Header -->
      <div class="receipt-header">
        <div class="header-left">
        ...
        <div class="header-right">
          <div class="receipt-badge">TUITION FEE RECEIPT</div>
          <div class="copy-badge">(${copyType})</div>
        </div>
      </div>
      ...
      <table class="grid-table">
        <tr>
          <td style="width: 50%;"><strong>RECEIPT NO.:</strong> ${data.receiptNo}</td>
          <td style="width: 50%;"><strong>DATE:</strong> ${dateStr}</td>
        </tr>
        <tr>
          <td style="width: 50%;"><strong>REGISTRATION NO.:</strong> ${data.registrationNo}</td>
          <td style="width: 50%;"><strong>STUDENT NAME:</strong> ${data.studentName}</td>
        </tr>
        <tr>
          <td colspan="2"><strong>COURSE / PROGRAM:</strong> ${data.courseTitle}</td>
        </tr>
      </table>
      ...
      <table class="grid-table amount-table">
        <thead>
          <tr>
            <th>DESCRIPTION / BILLING CYCLE</th>
            <th style="width: 160px; text-align: right;">AMOUNT</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>
              <strong>Monthly Tuition Fee — ${data.cycleLabel}</strong>
              ${data.transactionRef ? `<br/><span style="font-size: 10px; color: #555;">Ref/UTR: ${data.transactionRef}</span>` : ""}
              ${data.notes ? `<br/><span style="font-size: 10px; color: #555;">Note: ${data.notes}</span>` : ""}
            </td>
            <td style="text-align: right; font-weight: 600;">${amtFormatted}</td>
          </tr>
          <tr class="total-row">
            <td><strong>Total Amount Paid</strong></td>
            <td style="text-align: right;"><strong>${amtFormatted}</strong></td>
          </tr>
        </tbody>
      </table>
      ...
      <!-- Payment Mode -->
      <div class="payment-mode-line">
        <strong>Payment Mode:</strong> ${data.paymentMode || "UPI / Cash"} &nbsp;|&nbsp; <strong>Status:</strong> <span style="color: #057a55; font-weight: bold;">PAID (Verified)</span>
      </div>
      ...
      <div class="signatures-row">
        ...
        <div class="sig-col text-right">
          <div class="sig-line margin-left-auto"></div>
          <div>Authorized Neos Astra Signature</div>
        </div>
      </div>
    </div>
  `;
...
<head>
  <meta charset="UTF-8" />
  <title>Neos Astra Tuition Fee Receipt — ${data.receiptNo}</title>
  <style>
    @page { size: A4 portrait; margin: 10mm 12mm; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Times New Roman', Times, serif, 'Inter', sans-serif; color: #000; background: #fff; line-height: 1.3; }
    ...
    .header-right { width: 42%; text-align: right; font-family: 'Times New Roman', serif; }
    .receipt-badge { font-size: 14px; font-weight: bold; text-transform: uppercase; }
    .copy-badge { font-size: 10.5px; font-weight: bold; margin-top: 2px; }
...
<body>
  ...
  <!-- Dual Copy: Student Copy & Office Copy -->
  <div class="page">
    ${renderSingleMonthlyReceipt("STUDENT COPY")}

    <div class="dotted-divider">
      <span class="divider-text">✂ CUT HERE FOR OFFICE COPY</span>
    </div>

    ${renderSingleMonthlyReceipt("OFFICE COPY")}
  </div>
